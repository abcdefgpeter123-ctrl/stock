...
八項指標**全都是位置或斜率**（站上均線、均線方向），沒有任何一項在問
「到底有沒有前進」。均線落後於價格，所以一波大漲之後，就算指數橫向震盪一整個月，
價格仍在均線上、均線仍向上，分數可以一直停在大牛。
...
`market_status.js` 的 `evaluate()` 回傳多一個 `chop`，
等級是大牛／小牛且（效率比 < 0.15 或近 20 日淨變動絕對值 < 2%）時給出提示。
首頁與每日健檢都會顯示。

**2026/08/19 的實例**：分數 9.2（大牛），但近 20 日淨變動 −2.04%、效率比 0.05。
過去 5 年的大牛日裡，近 20 日淨變動為負的只佔 **3%**——是罕見狀態，不是誤判。

驗證（5 年逐日）：
- 2026/06 單向大漲那波：效率比 0.19–0.42、淨 +6.7~+14% → **不跳提示**
- 2026/08 這段：效率比 0.01–0.06、淨 −2~+2.4% → **每天跳**
- 多方日子中會跳的比例 32%

⚠️ **刻意不列入計分。** 回測顯示「大牛＋低效率」之後 60 日仍有 +6.49%／勝率 66.4%，
逐年只有 2/4 年比高效率差——沒有足夠證據支持降分。
它要解決的是「標籤與體感不符」，不是「分數預測不準」，這兩件事要分開。
